...
- **Theme:** catppuccin/tmux (mocha flavor)
- **Left side:** Session module
- **Right side:** DevPod session count (custom), directory, host, date/time

### Pane Border
- **Status:** Top (shows pane number, title, and git status)
- **Active border:** Catppuccin blue (`@thm_blue`)
- **Inactive border:** Catppuccin surface (`@thm_surface_0`)
- **Git status:** Shows branch, ahead/behind, staged, modified files (via gitmux)
...
All tmux configuration is generated by Home Manager from
`modules/home-manager/programs/tmux.nix`. The repo's `tmux/tmux.conf` is a
non-consumed reference stub — editing it has no effect. There is **no TPM**, so
there is no `Prefix + I` install step. After any change, apply with
`home-manager switch --flake .` and reload with `Prefix + r`.

### Add New Plugin
Edit the `programs.tmux.plugins` list in `modules/home-manager/programs/tmux.nix`
(plugins come from `pkgs.tmuxPlugins`):
```nix
...
### Change Prefix Key
The prefix is a declarative option. In your `home.nix`:
```nix
dev-config.tmux.prefix = "C-a";  # default
```
(Or set `programs.tmux.prefix` if consuming the module directly.)

### Customize Key Bindings (splits, etc.)
Edit the `extraConfig` block in `modules/home-manager/programs/tmux.nix`:
```tmux
