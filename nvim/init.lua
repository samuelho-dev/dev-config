...
require('lazy').setup(
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      formatters_by_ft = {
        ...
        -- Python formatters
        python = { 'ruff_format' },
        -- JavaScript/TypeScript formatters (prettier is most common)
  ...
  { -- Autocompletion
    ...
    opts = {
      ...
      sources = {
        ...
        providers = {
          minuet = { module = 'minuet.blink', score_offset = 120 },
  ...
  -- Multiple cursors (VS Code-style)
  {
    ...
    init = function()
      -- Customize default mappings to avoid conflicts
      -- Use <C-Down>/<C-Up> for vertical cursors (unchanged)
      -- Use <C-LeftMouse> for mouse cursor placement (unchanged)
      -- Change <C-n> to use <leader>m (m for multi-cursor) to avoid blink.cmp conflict
      vim.g.VM_maps = {
        ['Find Under'] = '<leader>m', -- Start multi-cursor, select word under cursor
        ['Find Subword Under'] = '<leader>m', -- Same for subwords
        -- Keep arrow keys for creating cursors vertically
        -- ['Add Cursor Down'] = '<C-Down>',  -- Default, kept
        -- ['Add Cursor Up'] = '<C-Up>',      -- Default, kept
      }

      -- Optional: Disable some default mappings if needed
      -- vim.g.VM_default_mappings = 0  -- Uncomment to disable ALL defaults (not recommended)

      -- Set the leader key for VM commands (default is \\)
      -- We keep it as \\ to avoid conflicts with Neo-tree toggle (\)
      -- vim.g.VM_leader = '\\\\'  -- Already default

      -- Customize theme to match tokyonight
      vim.g.VM_theme = 'iceblue' -- Options: 'iceblue', 'ocean', 'sand', 'purplegray'
  ...
  -- Indent guides with scope highlighting
  {
    ...
    opts = {
      indent = {
        char = '│', -- Vertical line character (can also use '▏', '┊', '┆')
        tab_char = '│',
      },
      scope = {
        enabled = true, -- Highlight current scope
        show_start = true, -- Show underline at scope start
        show_end = false, -- Don't show underline at scope end (cleaner)
        injected_languages = true, -- Support injected languages (e.g., JS in markdown)
        highlight = { 'Function', 'Label' }, -- Use colorscheme colors for scope
        priority = 1024,
      },
      exclude = {
        filetypes = {
  ...
  -- Auto-detects Obsidian vaults by scanning for .obsidian directories
  -- Only loads for markdown files inside detected vaults
  -- Regular markdown files outside vaults work normally
  (function()
    -- Helper: Find all Obsidian vaults by looking for .obsidian directories
    local function find_obsidian_vaults()
      local vaults = {}
      local search_paths = {
        vim.fn.expand '~/Documents',
        vim.fn.expand '~/Library/Mobile Documents/iCloud~md~obsidian',
        vim.fn.expand '~/Dropbox',
      ...
      for _, search_path in ipairs(search_paths) do
        if vim.fn.isdirectory(search_path) == 1 then
          -- Check if search_path itself is a vault
          if vim.fn.isdirectory(search_path .. '/.obsidian') == 1 then
            table.insert(vaults, search_path)
          end
          -- Search one level deep for vaults
          ...
          if handle then
            while true do
              ...
              if type == 'directory' then
                local potential_vault = search_path .. '/' .. name
                if vim.fn.isdirectory(potential_vault .. '/.obsidian') == 1 then
                  table.insert(vaults, potential_vault)
                end
              end
            end
          end
    ...
    -- Detect vaults
    local vault_paths = find_obsidian_vaults()

    -- Build workspaces config
    local workspaces = {}
    local events = {}
    for _, path in ipairs(vault_paths) do
      ...
      -- Add event patterns to only load for files in this vault
    ...
    -- Return plugin config (only if vaults found)
    return {
